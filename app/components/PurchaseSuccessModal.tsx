...
const PurchaseSuccessModal = ({ currentPurchase, onClose }: PurchaseSuccessModalProps) => {
    ...
    <div className='fixed inset-0 bg-[#808080] bg-opacity-50 flex items-center justify-center z-50 p-4'>
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-sm w-full">
            <div className="text-center mb-4">
                <div className="text-4xl">{currentPurchase.item.icon}</div>
                <h3 className="text-xl font-semibold mb-2">{currentPurchase.item.name}</h3>
            ...
            {currentPurchase.secret && (
                <div className="my-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-center">
                    <p className="text-sm font-semibold mb-1">Your secret code is:</p>
            ...
            <button onClick={onClose} className='mt-4 w-full tg-button cursor-pointer'>Close</button>
        </div>
    </div>
  )
}
